...
import { ParkingCircle, Coins, Star } from "lucide-react";
...
const spotIcon = (mode: "credit" | "commercial", available: boolean) => {
  const iconMarkup = renderToStaticMarkup(
    mode === "credit" ? (
      <ParkingCircle size={18} color="white" strokeWidth={2.5} />
    ) : (
...
export default function MapView({ spots }: { spots: ParkingSpot[] }) {
  ...
  const center = useMemo(() => pos ?? { lat: 27.7172, lng: 85.324 }, [pos]);
          ...
          <Popup>
            <div className="min-w-[170px]">
              ...
              <p className="mt-1 flex items-center gap-1 text-xs">
                ...
                {s.rating_count > 0 && (
                  <span className="flex items-center gap-0.5">
                    ·{" "}
                    <Star className="h-3 w-3 fill-signal-amber text-signal-amber" />
              ...
              <div className="mt-2 flex gap-2">
                <Link
                  href={`/booking/${s.id}`}
                  className="inline-block rounded-lg bg-night-800 px-3 py-1.5 text-xs font-medium text-white"
                >
                  Reserve
                </Link>
                <a
                  href={getDirectionsUrl(s.latitude, s.longitude)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block rounded-lg border border-night-900/15 px-3 py-1.5 text-xs font-medium text-night-800"
                >
                  Get there
                </a>
              </div>
