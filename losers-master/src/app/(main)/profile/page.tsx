...
import { Star, Car, Coins, ChevronRight, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="px-4 pt-8 pb-6">
      ...
      <Link
        ...
        className="mt-3 flex items-center justify-between rounded-xl2 bg-white p-4 shadow-card"
      >
        <span className="flex items-center gap-2 text-sm font-medium text-night-900">
          <ShieldCheck className="h-4 w-4 text-night-800/50" />
          Identity verification
        </span>
        <span className="flex items-center gap-1">
          <VerificationPill status={profile?.verification_status ?? "unverified"} />
          <ChevronRight className="h-4 w-4 text-night-800/40" />
        </span>
      </Link>
      ...
      {profile?.is_admin && (
        <Link
          href="/admin"
          className="mt-3 flex items-center justify-between rounded-xl2 bg-night-900 p-4 shadow-card"
        >
