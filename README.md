...
## 7. What's intentionally left as "future work" (per your own spec)
...
- **Server-triggered push notifications** (e.g. auto-notify a host when
  their spot is booked) — the OneSignal client SDK is wired in and ready to
  receive pushes; sending them automatically needs a small Supabase Edge
  Function calling the OneSignal REST API on `bookings` insert/update. Ask
